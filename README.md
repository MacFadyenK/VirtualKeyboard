...
## Abstract
The designed project is an EEG-driven virtual keyboard intended to aid those with impaired motor abilities and improve technology in non-verbal communication. The system integrates a brain–computer interface (BCI) to interpret electroencephalogram (EEG) signals from a database in order to find the associated character. To achieve a low energy, low latency design, a spiking neural network (SNN) classifier was implemented with field programmable gate array (FPGA) acceleration. An EEG dataset was preprocessed, reduced in feature space, and converted into spike trains. A three layer fully connected SNN was trained to classify signal presence. Character selection was done by calculating the probabilities of P300 signal presence per row/column and finding the intersection with the highest score. The trained SNN was implemented onto an Artix7 FPGA. Final character selection accuracy was calculated as 48.31 ± 22.15% on CPU and 4.76% on FPGA. CPU accuracy was significantly higher than the random weight classifier baseline (p << 0.05). The system had a total latency of 137.0 ± 30.9 ms for full pipeline on CPU and 113.2 ± 30.9 ms using FPGA acceleration. Energy consumption for the virtual keyboard was 109.21 μJ on CPU and 69.98 μJ on FPGA. A comparison with a traditional artificial neural network (ANN) with similar accuracy revealed that the SNN is lower energy but higher latency. The acceleration of BCI systems using FPGA shows promise for creating a more affordable and accessible device.
